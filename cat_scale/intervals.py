...
def collapse_visits_to_intervals(visits, interval_collapse_ms=DEFAULT_VISIT_INTERVAL_COLLAPSE_MS):
    ...
    dt = dt.astimezone(TZ_LOCAL)
    dt = dt.replace(hour=23, minute=59, second=59, microsecond=99999)
    end_ts = int(dt.timestamp() * 1000) + DEFAULT_VISIT_INTERVAL_COLLAPSE_MS
    ...
    if next_visit:
        for tick_ts in range(start_ts, end_ts, interval_collapse_ms):
            ...
            dt = datetime.utcfromtimestamp(int(tick_ts/1000))
            dt = dt.astimezone(TZ_LOCAL)
            ...
            while next_visit:
                ...
                dt = datetime.utcfromtimestamp(int(next_visit['start_timestamp']/1000))
                dt = dt.astimezone(TZ_LOCAL)
                ...
                # start a new interval
                    ...
                    tick = tick_start_ts
                    dt = datetime.utcfromtimestamp(int(tick/1000)).astimezone(TZ_LOCAL)
